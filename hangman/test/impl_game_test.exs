defmodule HangmanImplTest do
  ...
  test "new game returns lower case word" do
    ...
    assert game.turns_left == 7
    assert game.game_state == :initializing
    assert game.letters |> Enum.join() |> String.match?(~r/^[[:lower:]]+$/u)
  end

  test "new game always lower case" do
    game = Game.new_game()

    assert game.turns_left == 7
    assert game.game_state == :initializing
    assert game.letters |> Enum.all?(fn c -> String.match?(c, ~r/[a-z]/) end)
  end
  ...
  test "state doesn't change if a game is won or lost" do
    for state <- [:won, :lost] do
      ...
      {updated_game, _tally} = Game.make_move(game, "a")
